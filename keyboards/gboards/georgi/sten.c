...
uint32_t prefixState = 0;			// Prefix of a chord reported by the user
#ifndef NO_DEBUG
char debugMsg[32];
#endif
...
uint16_t repTimer 		= 0;
#ifndef REP_ATTACK
#define REP_ATTACK   750
#endif
#ifndef REP_INTERVAL
#define REP_INTERVAL 25
#endif
...
#ifndef NO_DEBUG
const char *order[] = {
  "FN", "PWR", "ST1", "ST2", "ST3", "ST4", "LNO", "RNO",
  "LSU", "LSD", "LT", "LK", "LP", "LW", "LH", "LR", "LA", "LO",
  "RE", "RU", "RF", "RR", "RP", "RB", "RL", "RG", "RT", "RS", "RD", "RZ", "RES1", "RES2"
};
void print_state(uint32_t chord) {
  if (chord == 0xFFFFFFFF) {
    uprintf("{ALL}");
    return;
  }
...
#else
void print_state(uint32_t chord) { return; }
#endif
...
void matrix_scan_user(void) {
  ...
  // Check timers
#ifndef NO_REPEAT
  if (repEngaged && timer_elapsed(repTimer) > REP_INTERVAL) {
  ...
  if (!repEngaged && timer_elapsed(repTimer) > REP_ATTACK) {
    repEngaged = true;
  }
#endif
};
...
void processChord(bool useFakeSteno) {
#ifndef NO_DEBUG
  uprintf("pC called, param: %d, cChord=", (uint16_t)useFakeSteno);
  print_state(cChord);
  uprintf(", chordState=[");
  for (int i = 0; i < chordIndex; i++) {
    print_state(chordState[i]);
    uprintf(", ");
  }
  print_state(chordState[chordIndex]);
  uprintf("]\n");
#endif
  ...
  debug_section(uprintf("bufChords=["));
  ...
  while (savedChord != mask) {
    ...
    uint32_t matchPrefix  = 0;

    for (int i = 0; i <= chordIndex; i++) {
      cChord = (chordState[i] & ~mask) | savedPrefix;
      if (cChord == 0)
        continue;
    ...
    mask |= longestChord;
    savedPrefix = matchPrefix;
    debug_section(
      if (bufLen != 0) {
        uprintf(", ");
      }
      print_state(longestChord);
    )
...
void REPORT_PREFIX(uint32_t prefix) {
  prefixState = prefix;
  return;
}
